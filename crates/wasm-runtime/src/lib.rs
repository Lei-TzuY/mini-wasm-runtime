...
use control::{
    build_control_map, read_block_signature, BlockSignature, ControlInfo, ControlKind, ControlMap,
    ExecControlFrame,
};
...
fn execute_simd(
    ...
    match subopcode {
        ...
        88..=91 => {
            ...
            ensure_runtime_memory_index(instance, memory_index)?;
            let lane = *code.get(*pc).ok_or(RuntimeError::ControlInvariant(
                "validated SIMD lane-store immediate is missing",
            ))?;
            *pc += 1;
            let (width, lane_limit) = match subopcode {
                88 => (1usize, 16u8),
            ...
            if lane >= lane_limit {
                return Err(RuntimeError::ControlInvariant(
                    "validated SIMD lane-store lane is out of bounds",
                ));
            }
            let vector = numeric::v128_from_stack(stack)?;
            let address = pop_runtime_memory_address(instance, stack, memory_index)?;
            let start = usize::from(lane) * width;
            instance.with_memory_index_mut(memory_index, |memory| match subopcode {
                88 => memory.store_i8(address, displacement, i32::from(vector[start])),
                89 => memory.store_i16(
                    address,
                    displacement,
                    i32::from(u16::from_le_bytes([vector[start], vector[start + 1]])),
                ),
                90 => memory.store_i32(
                    address,
                    displacement,
                    i32::from_le_bytes(vector[start..start + 4].try_into().expect("lane width")),
                ),
                91 => memory.store_i64(
                    address,
                    displacement,
                    i64::from_le_bytes(vector[start..start + 8].try_into().expect("lane width")),
                ),
                _ => unreachable!("matched SIMD lane-store opcode"),
            })?;
        }
        ...
        95 => {
            ...
            for lane in 0..2 {
                ...
                result[output_start..output_start + 8]
                    .copy_from_slice(&(input as f64).to_bits().to_le_bytes());
            }
            stack.push(Value::V128(Rc::new(result)));
        }
        ...
        263 => {
            // f64x2.relaxed_madd permits either fused or unfused evaluation.
            // Use ordinary multiply followed by add for a portable deterministic lowering.
            let c = numeric::v128_from_stack(stack)?;
            let b = numeric::v128_from_stack(stack)?;
            let a = numeric::v128_from_stack(stack)?;
        ...
        262 => {
            // f32x4.relaxed_nmadd permits either fused or unfused evaluation.
        ...
        270 => {
            // f32x4.relaxed_max permits implementation-defined choice for NaN and
            // signed-zero ties. Reuse deterministic f32x4.max-compatible semantics.
            let rhs = numeric::v128_from_stack(stack)?;
            let lhs = numeric::v128_from_stack(stack)?;
            let mut result = [0u8; 16];
            for lane in 0..4 {
        ...
        24 | 25 => {
            let lane = *code.get(*pc).ok_or(RuntimeError::ControlInvariant(
                "validated i16x8.extract_lane immediate is missing",
            ))?;
            *pc += 1;
            if lane >= 8 {
                return Err(RuntimeError::ControlInvariant(
                    "validated i16x8.extract_lane lane is out of bounds",
                ));
            }
            let vector = numeric::v128_from_stack(stack)?;
            let start = usize::from(lane) * 2;
            let word = u16::from_le_bytes(
                vector[start..start + 2]
                    .try_into()
                    .expect("validated i16x8 lane width"),
            );
            let value = if subopcode == 24 {
        ...
        26 => {
            let lane = *code.get(*pc).ok_or(RuntimeError::ControlInvariant(
                "validated i16x8.replace_lane immediate is missing",
            ))?;
            *pc += 1;
            if lane >= 8 {
                return Err(RuntimeError::ControlInvariant(
                    "validated i16x8.replace_lane lane is out of bounds",
                ));
            }
            let scalar = numeric::i32_from_stack(stack)?;
            let mut vector = numeric::v128_from_stack(stack)?;
            let start = usize::from(lane) * 2;
            vector[start..start + 2].copy_from_slice(&(scalar as u16).to_le_bytes());
            stack.push(Value::V128(Rc::new(vector)));
        }
        35..=44 => {
            let rhs = numeric::v128_from_stack(stack)?;
        ...
        77 => {
            let value = numeric::v128_from_stack(stack)?;
            let mut result = [0u8; 16];
            for (output, input) in result.iter_mut().zip(value) {
                *output = !input;
            }
            stack.push(Value::V128(Rc::new(result)));
        }
        78..=81 => {
            ...
            let mut result = [0u8; 16];
            for index in 0..16 {
                result[index] = match subopcode {
                    78 => lhs[index] & rhs[index],
                    79 => lhs[index] & !rhs[index],
                    80 => lhs[index] | rhs[index],
                    81 => lhs[index] ^ rhs[index],
                    _ => unreachable!("matched v128 binary bitwise opcode"),
                };
            }
            stack.push(Value::V128(Rc::new(result)));
        }
        82 => {
            ...
            let mut result = [0u8; 16];
            for index in 0..16 {
                result[index] = (lhs[index] & mask[index]) | (rhs[index] & !mask[index]);
            }
            stack.push(Value::V128(Rc::new(result)));
        }
        83 => {
        ...
        142 | 143 | 144 | 145 | 146 | 147 | 149 | 150 | 151 | 152 | 153 | 155 => {
            ...
            for lane in 0..8 {
                ...
                let value = match subopcode {
                    ...
                    143 => (lhs_lane as i16).saturating_add(rhs_lane as i16) as u16,
                    144 => lhs_lane.saturating_add(rhs_lane),
        ...
        206 | 209 | 213 => {
            ...
            for lane in 0..2 {
                let start = lane * 8;
                let lhs_lane =
                    u64::from_le_bytes(lhs[start..start + 8].try_into().expect("i64x2 lane width"));
                let rhs_lane =
                    u64::from_le_bytes(rhs[start..start + 8].try_into().expect("i64x2 lane width"));
                let value = match subopcode {
                    206 => lhs_lane.wrapping_add(rhs_lane),
                    209 => lhs_lane.wrapping_sub(rhs_lane),
                    213 => lhs_lane.wrapping_mul(rhs_lane),
                    _ => unreachable!("matched i64x2 arithmetic opcode"),
                };
        ...
        214..=219 => {
            ...
            for lane in 0..2 {
                let start = lane * 8;
                let lhs_lane =
                    i64::from_le_bytes(lhs[start..start + 8].try_into().expect("i64x2 lane width"));
                let rhs_lane =
                    i64::from_le_bytes(rhs[start..start + 8].try_into().expect("i64x2 lane width"));
                let matches = match subopcode {
                    214 => lhs_lane == rhs_lane,
                    215 => lhs_lane != rhs_lane,
                    216 => lhs_lane < rhs_lane,
                    217 => lhs_lane > rhs_lane,
                    218 => lhs_lane <= rhs_lane,
                    219 => lhs_lane >= rhs_lane,
                    _ => unreachable!("matched i64x2 comparison opcode"),
                };
                let mask = if matches { u64::MAX } else { 0 };
                result[start..start + 8].copy_from_slice(&mask.to_le_bytes());
            }
            stack.push(Value::V128(Rc::new(result)));
        }
        220..=223 => {
