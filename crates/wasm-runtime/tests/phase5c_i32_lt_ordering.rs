...
#[test]
fn i32_lt_s_matches_pinned_upstream_ordering_vectors() {
    for &(lhs, rhs, expected) in LT_S_CASES {
        assert_eq!(run(I32_LT_S, lhs, rhs), expected, "lt_s({lhs}, {rhs})");
    }
}

#[test]
fn i32_lt_u_matches_pinned_upstream_ordering_vectors() {
    for &(lhs, rhs, expected) in LT_U_CASES {
        assert_eq!(
            run(I32_LT_U, lhs, rhs),
            expected,
            "lt_u({lhs:#x}, {rhs:#x})"
        );
    }
