...
fn widening_loads_extend_each_lane_with_exact_signedness() {
    let bytes = [
        0x80, 0x7f, 0xff, 0x01, 0x00, 0xaa, 0x55, 0xfe, 0x00, 0x80, 0xff, 0x7f, 0xff, 0xff,
        0x34, 0x12, 0x00, 0x00, 0x00, 0x80, 0xff, 0xff, 0xff, 0x7f,
    ];

    assert_eq!(
        execute_memory32(1, 3, 0, &bytes),
        [0x80, 0xff, 0x7f, 0x00, 0xff, 0xff, 0x01, 0x00, 0x00, 0x00, 0xaa, 0xff, 0x55, 0x00, 0xfe, 0xff]
    );
    assert_eq!(
        execute_memory32(2, 3, 0, &bytes),
        [0x80, 0x00, 0x7f, 0x00, 0xff, 0x00, 0x01, 0x00, 0x00, 0x00, 0xaa, 0x00, 0x55, 0x00, 0xfe, 0x00]
    );
    assert_eq!(
        execute_memory32(3, 3, 8, &bytes),
        [0x00, 0x80, 0xff, 0xff, 0xff, 0x7f, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0x34, 0x12, 0x00, 0x00]
    );
    assert_eq!(
        execute_memory32(4, 3, 8, &bytes),
        [0x00, 0x80, 0x00, 0x00, 0xff, 0x7f, 0x00, 0x00, 0xff, 0xff, 0x00, 0x00, 0x34, 0x12, 0x00, 0x00]
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        [0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0x7f, 0x00, 0x00, 0x00, 0x00]
    );
}
...
fn splat_loads_repeat_the_exact_loaded_element() {
    let bytes = [
        0x80, 0x34, 0x12, 0x78, 0x56, 0x34, 0x12, 0x08, 0x07, 0x06, 0x05, 0x04, 0x03, 0x02,
        0x01,
    ];
