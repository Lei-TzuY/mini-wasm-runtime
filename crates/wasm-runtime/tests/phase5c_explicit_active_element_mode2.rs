...
fn explicit_table_index_active_element_mode_two_executes_end_to_end() {
    let parsed = parse_module(&explicit_element_module(0)).expect("mode-2 element segment must parse");
...
fn explicit_table_index_is_preserved_and_validated_before_instantiation() {
    let parsed = parse_module(&explicit_element_module(1)).expect("mode-2 element target must parse");
