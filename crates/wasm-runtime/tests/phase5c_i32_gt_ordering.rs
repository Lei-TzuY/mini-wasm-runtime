...
#[test]
fn i32_gt_s_matches_pinned_upstream_ordering_vectors() {
    for &(lhs, rhs, expected) in GT_S_CASES {
        assert_eq!(run(I32_GT_S, lhs, rhs), expected, "gt_s({lhs}, {rhs})");
    }
}

#[test]
fn i32_gt_u_matches_pinned_upstream_ordering_vectors() {
    for &(lhs, rhs, expected) in GT_U_CASES {
        assert_eq!(
            run(I32_GT_U, lhs, rhs),
            expected,
            "gt_u({lhs:#x}, {rhs:#x})"
        );
    }
