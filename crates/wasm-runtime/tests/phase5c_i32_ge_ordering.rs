...
#[test]
fn i32_ge_s_matches_pinned_upstream_ordering_vectors() {
    for &(lhs, rhs, expected) in GE_S_CASES {
        assert_eq!(run(I32_GE_S, lhs, rhs), expected, "ge_s({lhs}, {rhs})");
    }
}

#[test]
fn i32_ge_u_matches_pinned_upstream_ordering_vectors() {
    for &(lhs, rhs, expected) in GE_U_CASES {
        assert_eq!(
            run(I32_GE_U, lhs, rhs),
            expected,
            "ge_u({lhs:#x}, {rhs:#x})"
        );
    }
