...
#[test]
fn args_sizes_get_reports_configured_count_and_payload_bytes() {
    let memory = MemoryHandle::new(1, Some(1)).unwrap();
    let wasi = WasiPreview1::new().with_args(["app", "--flag"]);
    let mut vm = instantiate("args_sizes_get", 0, 4, &memory, &wasi);
    ...
    assert_eq!(read_u32(&memory, 4), 11);
}

#[test]
fn args_get_writes_pointer_table_and_nul_terminated_payload() {
    let memory = MemoryHandle::new(1, Some(1)).unwrap();
    let wasi = WasiPreview1::new().with_args(["app", "--flag"]);
    let mut vm = instantiate("args_get", 0, 32, &memory, &wasi);
    ...
    assert_eq!(read_u32(&memory, 4), 36);
    assert_eq!(memory.read(32, 11).unwrap(), b"app\0--flag\0");
}

#[test]
fn args_get_oob_payload_does_not_partially_write_pointer_table() {
    let memory = MemoryHandle::new(1, Some(1)).unwrap();
    memory.write(0, &[0xaa; 8]).unwrap();
    let wasi = WasiPreview1::new().with_args(["app", "--flag"]);
    let mut vm = instantiate("args_get", 0, 65_530, &memory, &wasi);
...
#[test]
fn args_sizes_get_preflights_both_output_words() {
    let memory = MemoryHandle::new(1, Some(1)).unwrap();
    memory.write(0, &[0xaa; 4]).unwrap();
    let wasi = WasiPreview1::new().with_args(["app"]);
    let mut vm = instantiate("args_sizes_get", 0, 65_534, &memory, &wasi);
...
#[test]
fn args_get_rejects_configured_limits_before_guest_memory_access() {
    let memory = MemoryHandle::new(1, Some(1)).unwrap();
    let wasi = WasiPreview1::new()
        .with_args(["app", "--flag"])
        .with_args_limits(1, 1024);
    let mut vm = instantiate("args_get", 65_535, 65_535, &memory, &wasi);
