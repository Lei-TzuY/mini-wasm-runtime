...
use wasm_wasi::{
    WasiPreview1, ERRNO_BADF, ERRNO_INVAL, ERRNO_NOTCAPABLE, ERRNO_SUCCESS, FDFLAGS_APPEND,
    RIGHTS_FD_FDSTAT_SET_FLAGS, RIGHTS_FD_SEEK, RIGHTS_FD_WRITE,
};
...
fn module() -> Vec<u8> {
    ...
    add_function_import(&mut imports, "fd_pwrite", 5);
    name(&mut imports, "env");
    name(&mut imports, "memory");
    imports.extend([2, 0, 1]);
    section(&mut module, 2, &imports);

    section(&mut module, 3, &[6, 0, 1, 2, 3, 4, 5]);
...
fn pwrite_args(fd: u32, offset: u64) -> [Value; 5] {
    [
        Value::I32(fd as i32),
        Value::I32(128),
        Value::I32(1),
...
fn append_flag_redirects_sequential_writes_to_eof_and_can_be_cleared() {
    ...
    assert_eq!(
        errno(
            &mut vm,
            "set_flags",
            &[Value::I32(fd as i32), Value::I32(i32::from(FDFLAGS_APPEND))],
        ),
        ERRNO_SUCCESS
    );
    assert_eq!(
        errno(&mut vm, "fdstat", &[Value::I32(fd as i32), Value::I32(320)],),
        ERRNO_SUCCESS
    );
...
fn append_can_be_requested_at_path_open() {
    ...
    assert_eq!(
        errno(
            &mut vm,
            "open",
            &open_args(rights, u32::from(FDFLAGS_APPEND))
        ),
        ERRNO_SUCCESS
    );
    let fd = u32::from_le_bytes(memory.read(100, 4).unwrap().try_into().unwrap());
...
#[test]
fn append_does_not_redirect_positioned_pwrite() {
    let memory = setup_memory();
    let wasi = WasiPreview1::new()
        .with_writable_preopen("/scratch")
        .unwrap()
    ...
    memory.write(256, b"P").unwrap();
    assert_eq!(errno(&mut vm, "pwrite", &pwrite_args(fd, 1)), ERRNO_SUCCESS);
    assert_eq!(wasi.file_snapshot("/scratch", "seed.bin").unwrap(), b"aPc");

    memory.write(256, b"Z").unwrap();
    assert_eq!(errno(&mut vm, "write", &write_args(fd)), ERRNO_SUCCESS);
    assert_eq!(wasi.file_snapshot("/scratch", "seed.bin").unwrap(), b"aPcZ");
}
...
fn set_flags_requires_capability_and_rejects_unsupported_flags_fail_closed() {
    ...
    assert_eq!(
        errno(
            &mut vm,
            "open",
            &open_args(RIGHTS_FD_WRITE | RIGHTS_FD_SEEK, 0)
        ),
        ERRNO_SUCCESS
    );
    let fd = u32::from_le_bytes(memory.read(100, 4).unwrap().try_into().unwrap());
    ...
    assert_eq!(
        errno(
            &mut vm,
            "set_flags",
            &[Value::I32(999), Value::I32(i32::from(FDFLAGS_APPEND))],
        ),
        ERRNO_BADF
    );
    ...
    assert_eq!(
        errno(
            &mut vm,
            "set_flags",
            &[Value::I32(3), Value::I32(i32::from(FDFLAGS_APPEND))],
        ),
        ERRNO_NOTCAPABLE
    );
