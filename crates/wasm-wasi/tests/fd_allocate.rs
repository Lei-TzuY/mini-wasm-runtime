...
fn module() -> Vec<u8> {
    ...
    function_type(&mut types, &[0x7f, 0x7e, 0x7e]);
    ...
    add_function_import(&mut imports, "fd_allocate", 1);
    ...
    for (export_name, function_index) in [("open", 4), ("allocate", 5), ("seek", 6), ("tell", 7)] {
...
#[test]
fn fd_allocate_extends_with_zero_fill_preserves_bytes_and_cursor() {
    ...
    let rights = RIGHTS_FD_ALLOCATE | RIGHTS_FD_SEEK | RIGHTS_FD_TELL;
    ...
    assert_eq!(read_u64(&memory, 120), 2);

    assert_eq!(
        errno(
            &mut vm,
            "allocate",
            &[Value::I32(fd as i32), Value::I64(5), Value::I64(3)],
        ),
        ERRNO_SUCCESS
    );
    assert_eq!(
        wasi.file_snapshot("/sandbox", "data.bin").unwrap(),
        vec![b'a', b'b', b'c', 0, 0, 0, 0, 0]
    );
    assert_eq!(
        errno(&mut vm, "tell", &[Value::I32(fd as i32), Value::I32(136)]),
        ERRNO_SUCCESS
    );
    assert_eq!(read_u64(&memory, 136), 2);

    assert_eq!(
        errno(
            &mut vm,
            "allocate",
            &[Value::I32(fd as i32), Value::I64(1), Value::I64(2)],
        ),
        ERRNO_SUCCESS
    );
    assert_eq!(
...
#[test]
fn fd_allocate_enforces_right_fd_class_overflow_and_size_limit_atomically() {
    let memory = MemoryHandle::new(1, Some(1)).unwrap();
    memory.write(64, b"data.bin").unwrap();
    let wasi = WasiPreview1::new()
        .with_writable_preopen("/sandbox")
    ...
    assert_eq!(
        errno(
            &mut vm,
            "allocate",
            &[Value::I32(fd as i32), Value::I64(0), Value::I64(4)],
        ),
        ERRNO_NOTCAPABLE
    );
    ...
    assert_eq!(
        errno(
            &mut vm,
            "allocate",
            &[Value::I32(999), Value::I64(0), Value::I64(1)],
        ),
        ERRNO_BADF
    );
    assert_eq!(
        errno(
            &mut vm,
            "allocate",
            &[Value::I32(1), Value::I64(0), Value::I64(1)],
        ),
        ERRNO_NOTCAPABLE
    );
    assert_eq!(
        errno(
            &mut vm,
            "allocate",
            &[Value::I32(3), Value::I64(0), Value::I64(1)],
        ),
        ERRNO_NOTCAPABLE
    );
    ...
    assert_eq!(
        errno(
            &mut vm,
            "open",
            &open_args(64, 8, 0, RIGHTS_FD_ALLOCATE, 100),
        ),
        ERRNO_SUCCESS
    );
    let fd = read_u32(&memory, 100);
    assert_eq!(
        errno(
            &mut vm,
            "allocate",
            &[Value::I32(fd as i32), Value::I64(-1), Value::I64(1)],
        ),
        ERRNO_FBIG
    );
    assert_eq!(
        errno(
            ...
            &[
                Value::I32(fd as i32),
                Value::I64(MAX_FILE_BYTES as i64),
                Value::I64(1),
            ],
        ),
        ERRNO_FBIG
...
#[test]
fn read_only_file_cannot_acquire_allocate_right() {
    let memory = MemoryHandle::new(1, Some(1)).unwrap();
    memory.write(64, b"data.bin").unwrap();
    let wasi = WasiPreview1::new()
...
#[test]
fn created_file_can_be_allocated_with_zero_fill() {
    let memory = MemoryHandle::new(1, Some(1)).unwrap();
    ...
    assert_eq!(
        errno(
            &mut vm,
            "open",
            &open_args(64, 7, OFLAGS_CREAT, RIGHTS_FD_ALLOCATE, 100),
        ),
        ERRNO_SUCCESS
    );
    let fd = read_u32(&memory, 100);
    assert_eq!(
        errno(
            &mut vm,
            "allocate",
            &[Value::I32(fd as i32), Value::I64(2), Value::I64(3)],
        ),
        ERRNO_SUCCESS
    );
    assert_eq!(
