...
old = """                        11 => {
                            ...
                            self.memory_fill(destination, value, length)?;
                        }
"""
rep(R, old, old + """                        12 => {
...
fn module(body: &[u8]) -> Vec<u8> {
    ...
    let mut c = vec![1, (body.len() + 1) as u8, 0]; c.extend_from_slice(body); section(&mut m, 10, &c);
    m
}
...
fn elem_drop_traps_followup_nonempty_init_atomically() {
    ...
    let table = TableHandle::new(2, Some(2)).unwrap();
    let mut vm = Instance::with_hosts(parse_module(&module(&body)).unwrap(), hosts(&table)).unwrap();
    assert!(matches!(vm.invoke_export("run", &[]), Err(RuntimeError::ElementSegmentSourceOutOfBounds { .. })));
...
fn validator_rejects_bad_element_index() {
    ...
    let table = TableHandle::new(2, Some(2)).unwrap();
    assert!(matches!(Instance::with_hosts(parse_module(&module(&body)).unwrap(), hosts(&table)), Err(RuntimeError::Validation(ValidationError::ElementIndexOutOfBounds { element_index: 1, .. }))));
}
