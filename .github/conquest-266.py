...
new = '''        265 => {
        ...
        256 => {'''
if old not in text:
    raise SystemExit('runtime opcode anchor not found')
text = text.replace(old, new, 1)
if '240..=265' not in text:
    raise SystemExit('runtime scanner frontier anchor not found')
runtime.write_text(text.replace('240..=265', '240..=266', 1))
...
if old not in text:
    raise SystemExit('validator opcode anchor not found')
validator.write_text(text.replace(old, new, 1))

for path in Path('crates/wasm-runtime/tests').glob('*.rs'):
    text = path.read_text()
    if '266' in text and 'UnsupportedPrefixedOpcode' in text:
        path.write_text(text.replace('266', '267'))

Path('crates/wasm-runtime/tests/simd_relaxed_laneselect_i16x8.rs').write_text(r'''use wasm_parser::parse_module;
...
Path('differential/tests/simd_relaxed_laneselect_i16x8.rs').write_text(r'''use wasm_parser::parse_module;
...
(module
  (func (export "all_a") (result i32)
    v128.const i16x8 -21846 -21846 -21846 -21846 -21846 -21846 -21846 -21846
    v128.const i16x8 21845 21845 21845 21845 21845 21845 21845 21845
...
#[test]
fn relaxed_i16x8_laneselect_matches_wasmtime_for_deterministic_masks() {
    ...
    assert_eq!(mini, vec![43690, 21845]);
    assert_eq!(reference, vec![43690, 21845]);
