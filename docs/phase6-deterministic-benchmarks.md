...
Controlled baselines are fail-closed. A baseline records its host ID, iteration/warmup/sample settings, policy constants, workload names, and per-workload median/MAD. Checking requires the same host ID, identical measurement settings, the exact current workload set, and the same compiled policy constants. Baseline files with schema drift, duplicate/stale/missing workloads, malformed values, or policy drift are rejected.
...
Baseline creation is also deliberate. `--write-baseline` uses create-new semantics and refuses to overwrite an existing file; refreshing a baseline therefore requires an explicit removal/new path plus normal code review. `benchmarks/baselines/` is reserved for reviewed measurements from identified controlled hosts.

The dedicated GitHub Actions benchmark smoke continues to run release-mode workloads with bounded iteration counts, plus formatting, Clippy, and unit tests for statistics/baseline parsing/policy. Shared GitHub-hosted runner timings are never fed into `--check-baseline`, so variable hosted hardware cannot create performance pass/fail decisions.
