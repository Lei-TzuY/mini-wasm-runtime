...
- SIMD `f32x4` ordered `min`/`max` plus pseudo-min/max (`pmin`/`pmax`) semantics are executable with NaN/equality behavior, signed-zero handling, typed validation, regressions, and Wasmtime differential coverage; `f64x2.abs`, `f64x2.neg`, and `f64x2.sqrt` are executable with typed validation, structured-control handling, focused regressions, and Wasmtime differential coverage; `f64x2.add`, `f64x2.sub`, `f64x2.mul`, and `f64x2.div` are executable with typed validation, structured-control handling, IEEE-754 edge regressions, and Wasmtime differential coverage; the adjacent `f64x2.min` opcode remains fail-closed.
