...
- [x] SIMD saturating `i16x8.add_sat_s` / `i16x8.add_sat_u` / `i16x8.sub_sat_s` / `i16x8.sub_sat_u` arithmetic with exact `v128, v128 -> v128` validation, independent signed/unsigned lane clipping, structured-control scanning, deterministic regressions, pinned Wasmtime differential evidence, and the still-unsupported `i16x8.extmul_low_i8x16_s` frontier kept fail-closed
