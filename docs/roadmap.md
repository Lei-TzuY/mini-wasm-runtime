...
- SIMD `i64x2` unary/reduction semantics are executable for wrapping `abs`/`neg`, `all_true`, and `bitmask`, with exact lane behavior, typed validation, structured-control scanning, focused regressions, and Wasmtime differential coverage; cross-width `i64x2.extend_*_i32x4_*` remains fail-closed for the next bounded slice.
...
- [x] SIMD `i8x16.shl` / `i8x16.shr_s` / `i8x16.shr_u` scalar-count shifts with exact `v128, i32 -> v128` validation, 3-bit masked counts, signed/unsigned byte-lane semantics, structured-control scanning, deterministic regressions, adjacent arithmetic fail-closed coverage, and pinned Wasmtime differential evidence
- [x] SIMD `i8x16` signed/unsigned saturating add/sub, min/max, and unsigned rounded average with exact `v128, v128 -> v128` validation, byte-lane semantics, structured-control scanning, focused fail-closed regressions, and Wasmtime differential evidence
