...
- [x] bounded non-following pathname metadata lookup via `path_filestat_get` with `PATH_FILESTAT_GET` attenuation, preopen- and opened-directory-relative regular/directory/symlink resolution, stable relational inode/link metadata, live sizes, deterministic logical-epoch timestamps, 64-byte fail-closed output preflight, and explicit `NOTSUP` for symlink-follow lookup flags
...
- [x] capability-scoped writable/create filesystem semantics with writable preopens, bounded `path_open(O_CREAT)`, sequential cursor-based `fd_write`, cursor-preserving `fd_pwrite`, sparse zero-fill, rights attenuation, and no ambient host paths\n- [x] bounded regular-file open-mode semantics via `path_open(O_CREAT|O_EXCL)` / `path_open(O_TRUNC)` with atomic existing-target exclusion, write-capability-gated shared-backing truncation, fail-closed output-memory preflight, and immediate visibility through already-open descriptors
...
- [x] bounded `poll_oneoff` readiness for already-ready clock subscriptions plus deterministic stdio/regular-file fd-read/fd-write events, with descriptor-rights enforcement, EOF HANGUP signaling, atomic guest-memory preflight, explicit no-scheduler/socket boundary, focused regressions, and pinned Wasmtime-WASI 37.0.3 differential evidence
...
- SIMD `i64x2` unary/reduction semantics are executable for wrapping `abs`/`neg`, `all_true`, and `bitmask`, with exact lane behavior, typed validation, structured-control scanning, focused regressions, and Wasmtime differential coverage.
...
- SIMD `f32x4` binary arithmetic is executable for `add`, `sub`, `mul`, and `div`, with typed validation, structured-control scanning, exact finite/IEEE regressions, and Wasmtime differential coverage; NaN-sensitive min/max remain fail-closed at subopcode 232.
...
- [x] pinned upstream memory64 conformance tranche from WebAssembly/spec `fc209c5ed8afc4dfeb9252024d217da3376c7a6f`, covering scalar i64-addressed active data/load-store plus bounded memory64 size/grow/OOB/max behavior with 3 modules, 38 executed assertions, zero filters, and the active-data offset-width correction exposed by the tranche
...
- [x] SIMD `i8x16.shuffle` / `i8x16.swizzle` byte-permutation semantics with cross-vector immediate selectors, repeated-lane support, dynamic-index zeroing, exact v128 typing, immediate bounds checks, structured-control scanning, deterministic regressions, and pinned Wasmtime differential evidence
- [x] SIMD `i8x16` comparison-mask family (`eq` / `ne`, signed/unsigned `lt` / `gt` / `le` / `ge`) with canonical `0xff`/`0x00` byte masks, exact `v128, v128 -> v128` validation, structured-control scanning, signed/unsigned boundary regressions, and pinned Wasmtime differential evidence
- [x] SIMD `i16x8.splat` / signed+unsigned `extract_lane` / `replace_lane` primitives with low-16-bit lane semantics, exact scalar/vector typing, bounds-checked 8-lane immediates, structured-control scanning, deterministic regressions, and pinned Wasmtime differential evidence
...
- [x] SIMD wrapping `i16x8.add` / `i16x8.sub` / `i16x8.mul` arithmetic with exact `v128, v128 -> v128` validation, independent 16-bit lane wrapping, structured-control scanning, adjacent saturating-opcode fail-closed coverage, deterministic regressions, and pinned Wasmtime differential evidence
- [x] SIMD saturating `i16x8.add_sat_s` / `i16x8.add_sat_u` / `i16x8.sub_sat_s` / `i16x8.sub_sat_u` arithmetic with exact `v128, v128 -> v128` validation, independent signed/unsigned lane clipping, structured-control scanning, deterministic regressions, and pinned Wasmtime differential evidence
- [x] SIMD remaining same-width `i16x8` scalar/ALU surface (`abs`, `neg`, `q15mulr_sat_s`, `all_true`, `bitmask`, scalar-count `shl` / `shr_s` / `shr_u`, signed/unsigned `min` / `max`, and `avgr_u`) with exact scalar/vector typing, masked shift counts, structured-control scanning, deterministic regressions, pinned Wasmtime differential evidence, and cross-width narrowing left fail-closed for a later bounded slice
- [x] SIMD `i8x16.abs` / `i8x16.neg` / `i8x16.popcnt` plus `i8x16.all_true` / `i8x16.bitmask` scalar reductions with wrapping signed-byte unary semantics, per-byte population counts, canonical scalar reductions, exact v128 typing, structured-control scanning, deterministic regressions, and pinned Wasmtime differential evidence
- [x] SIMD `i8x16.shl` / `i8x16.shr_s` / `i8x16.shr_u` scalar-count shifts with exact `v128, i32 -> v128` validation, 3-bit masked counts, signed/unsigned byte-lane semantics, structured-control scanning, deterministic regressions, and pinned Wasmtime differential evidence
- [x] SIMD `i8x16` signed/unsigned saturating add/sub, min/max, and unsigned rounded average with exact `v128, v128 -> v128` validation, byte-lane semantics, structured-control scanning, focused regressions, and Wasmtime differential evidence
...
- SIMD `f32x4` ordered `min`/`max` plus pseudo-min/max (`pmin`/`pmax`) semantics are executable with NaN/equality behavior, signed-zero handling, typed validation, regressions, and Wasmtime differential coverage; `f64x2.abs`, `f64x2.neg`, and `f64x2.sqrt` are executable with typed validation, structured-control handling, focused regressions, and Wasmtime differential coverage; `f64x2.add`, `f64x2.sub`, `f64x2.mul`, and `f64x2.div` are executable with typed validation, structured-control handling, IEEE-754 edge regressions, and Wasmtime differential coverage; `f64x2.min` and `f64x2.max` are executable with ordered NaN propagation, signed-zero handling, typed validation, structured-control handling, and focused regressions; `f64x2.pmin` and `f64x2.pmax` are executable with left-preserving unordered/equal behavior, exact `v128, v128 -> v128` validation, structured-control scanning, deterministic signed-zero/NaN/type-confusion regressions, and Wasmtime differential evidence; subopcode 248 remains fail-closed.

- SIMD terminal conversions (subopcodes 248-255) are executable across saturating f32/f64-to-i32 lanes, signed/unsigned integer-to-float lanes, the f64x2 low-lane conversion forms, typed validation, structured-control scanning, focused regressions, and Wasmtime differential coverage.
