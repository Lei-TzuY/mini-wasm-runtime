...
'''
unsigned_block = signed_block + '''        260 => {
...
'''
replace_once(runtime, signed_block, unsigned_block)
replace_once(runtime, "                    | 240..=259\n", "                    | 240..=260\n")
...
'''
validator_260 = validator_259 + '''                    260 => {
                        pop_expect(&mut stack, &controls, ValueType::V128, function, offset)?;
                        stack.push(ValueType::V128);
                    }
'''
replace_once(validator, validator_259, validator_260)

# Advance every existing fail-closed relaxed-SIMD frontier from 260 to 261.
# At this point main has no executable 260 coverage; its occurrences are frontier sentinels.
for path in (root / "crates/wasm-runtime/tests").glob("*.rs"):
    text = path.read_text()
    if "260" not in text:
        continue
    text = text.replace("subopcode: 260", "subopcode: 261")
    text = text.replace("simd(&mut code, 260)", "simd(&mut code, 261)")
    text = text.replace("simd(&mut instructions, 260)", "simd(&mut instructions, 261)")
    text = text.replace("simd(&mut i, 260)", "simd(&mut i, 261)")
    text = text.replace("push_simd(&mut instructions, 260)", "push_simd(&mut instructions, 261)")
    text = text.replace("push_simd(&mut i, 260)", "push_simd(&mut i, 261)")
    text = text.replace("next f64x2 opcode remains outside this slice", "next relaxed SIMD opcode remains outside this slice")
    text = text.replace("f32x4 frontier remains outside this slice", "relaxed SIMD frontier remains outside this slice")
    path.write_text(text)

runtime_test = root / "crates/wasm-runtime/tests/simd_relaxed_trunc_f64x2_u_zero.rs"
...
fn uses_permitted_saturating_choices_for_special_lanes() {
    ...
    assert_eq!(run_lane([f64::NAN, f64::INFINITY], 1), -1);
    assert_eq!(run_lane([f64::NEG_INFINITY, -42.9], 0), 0);
    assert_eq!(run_lane([f64::NEG_INFINITY, -42.9], 1), 0);
    assert_eq!(run_lane([4294967295.0, 42.9], 0), -1);
    assert_eq!(run_lane([4294967295.0, 42.9], 1), 42);
}
...
diff_test = root / "differential/tests/simd_relaxed_trunc_f64x2_u_zero.rs"
...
(module
  ...
  (func (export "max") (result i32)
    v128.const f64x2 4294967295 0
    i32x4.relaxed_trunc_f64x2_u_zero
    i32x4.extract_lane 0)
  (func (export "neg") (result i32)
    v128.const f64x2 -1 0
    i32x4.relaxed_trunc_f64x2_u_zero
    i32x4.extract_lane 0)
...
"#;
const EXPORTS: [&str; 6] = ["a", "b", "max", "neg", "z2", "z3"];
...
fn matches_wasmtime_on_deterministic_lanes() {
    ...
    let expected = vec![1, 12345, -1, 0, 0, 0];
